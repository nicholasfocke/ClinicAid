...
interface ExtratoDespesasOptions {
  titulo: string;
  colunas: string[];
...
export const gerarExtratoDespesas = async ({
  titulo,
  colunas,
  dados,
  nomeArquivo = 'extrato-despesas.pdf',
  periodo = '',
  status = '',
  filtroMes = '',
  totalDespesas = '',
}: ExtratoDespesasOptions) => {
    ...
    content: [
      ...
      { text: titulo, style: 'reportTitle' },
      {
        columns: [
          ...
          { text: `Total: ${dados.length} despesas`, style: 'info', alignment: 'right' },
        ],
        margin: [0, 0, 0, 10],
      },
      {
      ...
      {
        text: '\nEste extrato de despesas foi gerado automaticamente pelo sistema ClinicAid.',
        style: 'footer',
      },
    ].filter(Boolean),
    styles: {
